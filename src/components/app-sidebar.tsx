...
import { Activity, BarChart3, BookOpen, ClipboardCheck, LayoutDashboard, Settings, ShieldCheck } from "lucide-react";
...
export function AppSidebar() {
  ...
  return <aside className="fixed inset-y-0 left-0 z-40 hidden w-[72px] flex-col items-center border-r border-white/10 bg-zinc-950/70 py-5 backdrop-blur-2xl md:flex" aria-label="Primary navigation">
    ...
    <div className="flex flex-col items-center gap-3 border-t border-white/10 pt-4"><span className="rounded-xl p-3 text-zinc-600" title="Live governance signals"><Activity className="h-4 w-4" /></span><span className="rounded-xl p-3 text-zinc-600" title="Settings"><Settings className="h-4 w-4" /></span></div>
  </aside>;
}
